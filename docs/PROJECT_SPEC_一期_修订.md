...
- 命名空间规则（2026-08-18 迭代）：命名空间级资源命令必须携带 `-n/--namespace`（含 `-n=`/`--namespace=` 形式）；命令自带但与 Finding 命名空间不一致 → 丢弃（防 LLM 编造）；缺省 → 自动补全 Finding 命名空间（保证命令复制即用）；显式 `-A`/`--all-namespaces` 豁免；`logs` 动词按 Pod 处理。
